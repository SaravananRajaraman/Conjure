...
test("MessageList shows empty state when no messages", () => {
  ...
  expect(
    screen.getByText("Describe a component to get started")
  ).toBeDefined();
  expect(
    screen.getByText("I can build buttons, forms, cards, dashboards and more")
  ).toBeDefined();
});
...
test("MessageList applies correct styling for user vs assistant messages", () => {
  ...
  // User messages should have the accent background
  expect(userMessage?.className).toContain("bg-sky-600");
  expect(userMessage?.className).toContain("text-white");

  // Assistant messages should use the card surface tokens
