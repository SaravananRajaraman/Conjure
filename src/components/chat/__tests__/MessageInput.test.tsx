...
vi.mock("@/lib/contexts/framework-context", () => ({
  useFramework: () => ({ framework: "react", setFramework: vi.fn() }),
}));
...
test("applies correct CSS classes based on loading state", () => {
  ...
  expect(submitButton.className).toContain("hover:bg-sky-500/10");
...
test("textarea has correct styling classes", () => {
  ...
  expect(textarea.className).toContain("focus:ring-sky-500/20");
});
