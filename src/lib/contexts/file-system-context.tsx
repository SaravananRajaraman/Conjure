  ...
  const handleToolCall = useCallback(
    (toolCall: ToolCall) => {
        ...
        switch (command) {
          case "create":
            if (path && file_text !== undefined) {
              const existed = fileSystem.exists(path);
              const result = fileSystem.createFileWithParents(path, file_text);
              if (!result.startsWith("Error:")) {
                // Overwriting an existing file is an update, not a create, so
                // route through updateFile to refresh the preview/editor.
                if (existed) {
      ...
      if (toolName === "file_manager" && args) {
        const { command, path, new_path, file_text } = args;
