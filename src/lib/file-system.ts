...
export class VirtualFileSystem {
  ...
  createFileWithParents(path: string, content: string = ""): string {
    ...
    // If something already exists at this path, overwrite it (upsert). The
    // generation model frequently re-issues `create` to rewrite a component
    // wholesale on a follow-up edit; erroring here would silently drop the
    // change and leave the preview unchanged, so treat create as an upsert.
    if (this.exists(normalized)) {
      const existing = this.getNode(normalized);
      if (existing && existing.type === "directory") {
        return `Error: Cannot create file, a directory exists at: ${normalized}`;
      }
