...
export function buildFileManagerTool(fileSystem: VirtualFileSystem) {
  return tool({
    description:
      'Create, rename, or delete files or folders in the file system. Rename can be used to "move" a file. Rename will recursively create folders as required.',
    parameters: z.object({
      command: z
      ...
      file_text: z
        ...
        .optional()
        .nullable()
        .describe("The file content. Only provide when creating a file."),
    }),
    execute: async ({ command, path, new_path, file_text }) => {
      if (command === "create") {
