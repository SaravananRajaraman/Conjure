...
test("createFileWithParents overwrites an existing file (upsert)", () => {
  const fs = new VirtualFileSystem();
  fs.createFile("/test.txt", "content");

  const result = fs.createFileWithParents("/test.txt", "new content");
  expect(result).toBe("File updated: /test.txt");
  expect(fs.readFile("/test.txt")).toBe("new content");
});

test("createFileWithParents returns error when a directory exists at the path", () => {
  const fs = new VirtualFileSystem();
  fs.createDirectory("/components");

  const result = fs.createFileWithParents("/components", "content");
  expect(result).toBe(
    "Error: Cannot create file, a directory exists at: /components"
  );
});
